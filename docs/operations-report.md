...
独立 `operations-report` 服务每次同步结束后等待 600 秒，直接只读访问分析库，不依赖公网 Grafana。复用打包时的已提交 `gameplay-overview.json` 统计 SQL，同步 20 项核心指标，分别展示 WebGL+Windows 跨平台去重汇总、WebGL、Windows，共 60 条固定记录。使用与用户分享链接一致的过滤：全部渠道、全部版本、全部批次、测试数据 auto、行为累计、渠道会话窗口近 7 天。日期为北京时间。各平台/渠道人数不可相加；累计值不能按刷新时间解释成当日新增。
...
凭证复用服务器 `.env` 中的 `FEISHU_BOT_API_KEY`、`FEISHU_BOT_API_SECRET`、`FEISHU_ADMIN_ID`（可选 `FEISHU_ADMIN_MEMBER_TYPE`，`FEISHU_OPERATIONS_MEMBER_ID` 可指定报表协作者邮箱）。只为指定报表协作者或现有管理员授予新报表编辑权限，关闭飞书授权通知，不发送群消息。SQL 按文本缓存，一轮 3 个范围、固定 20 指标，每个范围重复 SQL 只执行一次；整轮查询 90 秒超时，单服务最多 0.5 CPU、512 MB 内存，失败等待下个周期，锁防止重入。
...
`tools/setup_operations_dashboard.py --plan-only` 生成27个组件配置：20指标卡、2平台对比、1渠道对比、3每日趋势、1说明。使用官方 Lark CLI 1.0.96，通过应用身份操作已经创建的 Base。实际搭建需应用发布 `base:dashboard:read`、`base:dashboard:create`、`base:dashboard:update`、`base:table:read`、`base:field:read` 权限。当前数据同步沿用既有 bitable v1 权限，仪表盘授权是独立前置条件。令牌仅经环境注入CLI，不打印或落盘。
...
网页版渠道指标使用明确上报的 `distribution_channel=web`，与 WebGL 运行平台分开；不把历史未上报渠道的 WebGL 会话改标为 web。Grafana 渠道行与飞书渠道图均包含网页版。新增指标使每轮最多增加三个固定范围的聚合查询，沿用90秒整轮查询超时、单实例锁与完成后600秒重试间隔。
